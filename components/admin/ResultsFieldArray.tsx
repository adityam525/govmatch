...
interface ResultsFieldArrayProps {
  items: ResultDraft[];
  onChange: (items: ResultDraft[]) => void;
...
const empty: ResultDraft = { title: '', resultType: 'FINAL', releaseDate: '', resultLink: '', examLevelId: '' };

export default function ResultsFieldArray({ items, onChange, examLevels }: ResultsFieldArrayProps) {
  const update = (i: number, key: keyof ResultDraft, value: string) => {
    const next = [...items];
  ...
  return (
    <div className="md:col-span-2 border-t border-neutral-100 pt-5 mt-2">
      ...
      {items.length === 0 && <p className="text-xs text-neutral-400 mb-3">No results added yet.</p>}
          ...
          <div key={i} className="border border-neutral-200 rounded-lg p-3 relative">
            ...
            <div className="grid md:grid-cols-2 gap-2 pr-6">
              <input
                ...
                placeholder="Title, e.g. Final Result"
                className="text-sm border border-neutral-200 rounded-md px-3 py-2 outline-none focus:border-primary-500 md:col-span-2"
              />
              <div>
                <label className="block text-xs font-medium text-neutral-600 mb-1">Result Type</label>
                <select value={item.resultType} onChange={(e) => update(i, 'resultType', e.target.value)} className="w-full text-sm border border-neutral-200 rounded-md px-3 py-2 outline-none focus:border-primary-500">
              ...
              <input
                value={item.resultLink}
                onChange={(e) => update(i, 'resultLink', e.target.value)}
