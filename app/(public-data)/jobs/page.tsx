...
export default async function JobsPage({ searchParams }: JobsPageProps) {
  ...
  const qualificationSlugs = params.qualifications?.split(',').filter(Boolean) ?? [];
  const organizationIds = params.organizations?.split(',').filter(Boolean) ?? [];
  ...
  let notifications = await prisma.notification.findMany({
    where: {
      ...
      ...(qualificationSlugs.length > 0
        ? { posts: { some: { qualification: { slug: { in: qualificationSlugs } } } } }
        : {}),
    },
    include: {
